...
> 修复本地前端代理配置避免默认接入错误后端；为费用、合同、访客、餐食、观察等详情接口补齐家属 motherIds 权限校验和脱敏汇总，确保家属不能通过详情 id 读取未绑定产妇或内部流程明细。
...
6. 宝宝异常观察的内部讨论和原始数据仅医护人员可见，家属端仅展示结论性信息
7. 未授权访客或超人数探视自动标记为特殊探视，需护士长审批后方可入场
8. 探视签退时需记录是否影响护理计划，便于后续护理安排调整
